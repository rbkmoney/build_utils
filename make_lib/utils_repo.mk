...
$(IMAGES_SHARED)/portage/.git: .git
	$(if $(BAKKA_SU_PRIVKEY),SSH_PRIVKEY=$(shell echo $(BAKKA_SU_PRIVKEY) | sed -e 's|%|%%|g'),) UTILS_PATH=$(UTILS_PATH) \
	"$(REPO_INIT)" "$(IMAGES_SHARED)/portage" "$(BAKKA_SU_URI_PREFIX)/gentoo-mirror"

# overlays
$(IMAGES_SHARED)/overlays/rbkmoney/.git: .git
	$(if $(GITHUB_PRIVKEY),SSH_PRIVKEY=$(shell echo $(GITHUB_PRIVKEY) | sed -e 's|%|%%|g'),) UTILS_PATH=$(UTILS_PATH) \
